import { bind, type Function, type Stringer } from "./core.ts";
 ...
 *
 */
export interface View extends Function<Node, Option<DocumentFragment>> {}
...
function applyObject<N extends Node>(
  ...
  for (const key in option) {
    ...
    if (isProperty<N, typeof key, N[typeof key]>(property)) {
      property.applyTo(node, key);
    } else {
 ...
 * @returns
 */
export function isNode(target: object): target is Node {
